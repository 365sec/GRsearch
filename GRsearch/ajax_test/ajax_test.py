...
def ipv4_select(request):
    ...
    if filter == "":
        ...
        filter_field_value = search_list[1]
        if search_content in es_china:
                content = json.dumps(ipv4_es_china_with_filter(search_content, page, s_type, filter_field, filter_field_value))
                return HttpResponse(content)                
        else:
                if search_content in es_list:
                    content = json.dumps(ipv4_es_with_filter(search_content, page, s_type, filter_field, filter_field_value))
                    return HttpResponse(content)
                else:
                    if ":" in search_content:
                        if "AND" in search_content:
                            content = json.dumps(ipv4_with_and_with_filter(search_content, page, s_type, filter_field, filter_field_value))
                            return HttpResponse(content)
                        elif "OR" in search_content:
                            content = json.dumps(ipv4_with_or_with_filter(search_content, page, s_type, filter_field, filter_field_value))
                            return HttpResponse(content)
                        elif "NOT" in search_content:
                            content = json.dumps(ipv4_with_not_with_filter(search_content, page, s_type, filter_field, filter_field_value))
                            return HttpResponse(content)
                        else :
                            content = json.dumps(ipv4_with_field_with_filter(search_content, page, s_type, filter_field, filter_field_value))
                            return HttpResponse(content)
                    else:
                        content = json.dumps(ipv4_with_content_with_filter(search_content, page, s_type, filter_field, filter_field_value))
                        return HttpResponse(content)
...
def ipv4_with_content(search_content, page, s_type):
    ...
    return context                         
def ipv4_es_china_with_filter(search_content, page, s_type, filter_field, filter_field_value):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    response = client.search(
    ...
    return context
def ipv4_es_with_filter(search_content, page, s_type, filter_field, filter_field_value):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    response = client.search(
    ...
    return context
def ipv4_with_and_with_filter(search_content, page, s_type, filter_field, filter_field_value):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    reg1 = re.compile(r'\s+AND\s+')  # 分割查询语句
    ...
    return context
def ipv4_with_or_with_filter(search_content, page, s_type, filter_field, filter_field_value):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    reg1 = re.compile(r'\s+OR\s+')  # 分割查询语句
    ...
    return context
def ipv4_with_not_with_filter(search_content, page, s_type, filter_field, filter_field_value):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    reg1 = re.compile(r'\s+NOT\s+')  # 分割查询语句
    ...
    return context
def ipv4_with_field_with_filter(search_content, page, s_type, filter_field, filter_field_value):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    reg2 = re.compile(r'\s*:\s*')
    ...
    return context 
def ipv4_with_content_with_filter(search_content, page, s_type, filter_field, filter_field_value):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    response = client.search(
