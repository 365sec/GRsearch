...
def ipv4_aggs(request):
    ...
    if filter == '':
        ...
        elif search_content in es_china:
...
def ipv4_es_china_aggs(search_content):
    ...
    protocol_list = [] 
    protocol_dict = {}
    protocol_dict["protocol_name"]="9200/http"
    protocol_dict["count"]=total_nums
    protocol_dict["percent"] = 100
    ...
    aggs_content["protocol_list"] = protocol_list
...
def ipv4_aggs_with_and(search_content):
    ...
    for bucket in aggs["aggregations"]["protocol"]["buckets"]: 
        if bucket["key"] != "":
            protocol_dict = {}
            protocol_dict["protocol_name"]=bucket["key"]
            protocol_dict["count"]=bucket["doc_count"]
            protocol_dict["percent"] = (float(bucket["doc_count"])/float(total_nums))*100
            protocol_list.append(protocol_dict)
        else:
            signal4 += 1
    if signal4 == 0:
...
def ipv4_aggs_with_and_with_filter(search_content,filter, field_dict):
    ...
    if protocol_status == 1:
        protocol_dict = {}
        protocol_dict["protocol_name"]=protocol_value
        protocol_dict["count"]=total_nums
        protocol_dict["percent"] = 100
        protocol_list.append(protocol_dict)
    else:
...
def ipv4_select(request):
    ...
    if filter == "":
        if search_content in es_china:
                content = json.dumps(ipv4_es_china(search_content, page, s_type))
                return HttpResponse(content)                
        else:
                if search_content in es_list:
                    content = json.dumps(ipv4_es(search_content, page, s_type))
                    return HttpResponse(content)
                else:
                    if ":" in search_content:
                        if "AND" in search_content:
                            content = json.dumps(ipv4_with_and(search_content, page, s_type))
                            return HttpResponse(content)
                        elif "OR" in search_content:
                            content = json.dumps(ipv4_with_or(search_content, page, s_type))
                            return HttpResponse(content)
                        elif "NOT" in search_content:
                            content = json.dumps(ipv4_with_not(search_content, page, s_type))
                            return HttpResponse(content)
                        else :
                            content = json.dumps(ipv4_with_field(search_content, page, s_type))
                            return HttpResponse(content)
                    else:
                        content = json.dumps(ipv4_with_content(search_content, page, s_type))
                        return HttpResponse(content)
    else:
        field_dict = {
                          "全文" : "_all",
        ...
        field_dict = json.loads(field_dict)
        if search_content in es_china:
                content = json.dumps(ipv4_es_china_with_filter(search_content, page, s_type, filter, field_dict))
                return HttpResponse(content)                
        else:
                if search_content in es_list:
                    content = json.dumps(ipv4_es_with_filter(search_content, page, s_type, filter, field_dict))
                    return HttpResponse(content)
                else:
                    if ":" in search_content:
                        if "AND" in search_content:
                            content = json.dumps(ipv4_with_and_with_filter(search_content, page, s_type, filter, field_dict))
                            return HttpResponse(content)
                        elif "OR" in search_content:
                            content = json.dumps(ipv4_with_or_with_filter(search_content, page, s_type, filter, field_dict))
                            return HttpResponse(content)
                        elif "NOT" in search_content:
                            content = json.dumps(ipv4_with_not_with_filter(search_content, page, s_type, filter, field_dict))
                            return HttpResponse(content)
                        else :
                            content = json.dumps(ipv4_with_field_with_filter(search_content, page, s_type, filter, field_dict))
                            return HttpResponse(content)
                    else:
                        content = json.dumps(ipv4_with_content_with_filter(search_content, page, s_type, filter, field_dict))
                        return HttpResponse(content)
def ipv4_es_china(search_content, page, s_type):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    ...
    return context
def ipv4_es(search_content, page, s_type):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    response = client.search(
    ...
    return context
def ipv4_with_and(search_content, page, s_type):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    reg1 = re.compile(r'\s+AND\s+')  # 分割查询语句
    ...
    return context
def ipv4_with_or(search_content, page, s_type):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    reg1 = re.compile(r'\s+OR\s+')  # 分割查询语句
    ...
    return context
def ipv4_with_not(search_content, page, s_type):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    reg1 = re.compile(r'\s+NOT\s+')  # 分割查询语句
    ...
    return context
def ipv4_with_field(search_content, page, s_type):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    reg2 = re.compile(r'\s*:\s*')
    ...
    return context 
def ipv4_with_content(search_content, page, s_type):
    current_page = page
    last_page = page - 1
    next_page = page + 1
    response = client.search(
    ...
    return context                         
def ipv4_es_china_with_filter(search_content, page, s_type, filter, field_dict):
    code = str(type(filter))
    if code == "<type 'unicode'>":
        filter = filter.encode("utf-8") 
        filter = json.loads(filter)
    ...
    return context
def ipv4_es_with_filter(search_content, page, s_type, filter, field_dict):
    code = str(type(filter))
    if code == "<type 'unicode'>":
        filter = filter.encode("utf-8") 
        filter = json.loads(filter)
    ...
    return context
def ipv4_with_and_with_filter(search_content, page, s_type, filter, field_dict):
    code = str(type(filter))
    if code == "<type 'unicode'>":
        filter = filter.encode("utf-8") 
        filter = json.loads(filter)
    ...
    return context
def ipv4_with_or_with_filter(search_content, page, s_type, filter, field_dict):
    code = str(type(filter))
    if code == "<type 'unicode'>":
        filter = filter.encode("utf-8") 
        filter = json.loads(filter)
    ...
    return context
def ipv4_with_not_with_filter(search_content, page, s_type, filter, field_dict):
    code = str(type(filter))
    if code == "<type 'unicode'>":
        filter = filter.encode("utf-8") 
        filter = json.loads(filter)
    ...
    return context
def ipv4_with_field_with_filter(search_content, page, s_type, filter, field_dict):
    code = str(type(filter))
    if code == "<type 'unicode'>":
        filter = filter.encode("utf-8") 
        filter = json.loads(filter)
    ...
    return context 
def ipv4_with_content_with_filter(search_content, page, s_type, filter, field_dict):
    code = str(type(filter))
    if code == "<type 'unicode'>":
        filter = filter.encode("utf-8") 
        filter = json.loads(filter)
